 ...
 */
public final class AplosFaceletCache extends FaceletCache<Facelet> {
	private Map<String,Boolean> urlListensToEvent = new HashMap<String,Boolean>();
     ...
     */
	AplosFaceletCache(final long refreshPeriod) {
        ...
        ConcurrentCache.Factory<URL, Record> faceletFactory =
            ...
            public Record newInstance(final URL key) throws IOException {
            	...
            	long lastModified = 0;
            	try {
            		lastModified = checkExpiry ? Util.getLastModified(key) : 0;
        ...
        if (refreshPeriod == 0) {
            ...
            ExpiringConcurrentCache.ExpiryChecker<URL, Record> checker = 
                (refreshPeriod > 0) ? new ExpiryChecker() : new AlmostNeverExpired();
            _faceletCache =
    ...
    public Facelet getFacelet(URL url) throws IOException {
		...
		if( urlListensToEvent.get( urlPath ) == null || urlListensToEvent.get( urlPath ) ) {
			urlListensToEvent.put( urlPath, ApplicationUtil.getAplosModuleFilterer().getFaceletEvent(urlPath) );
		}
    ...
    public void expireUrlFromCache(URL url) throws IOException {
        com.sun.faces.util.Util.notNull("url", url);
        Record record;
        
        try {
        	record = _faceletCache.get(url);
        	if( record != null ) {
        		record.setToExpire(true);
        	}
        } catch (ExecutionException e) {
            _unwrapIOException(e);
        }
    ...
    private static class Record {
        ...
        void setToExpire( boolean isSetToExpire ) {
        	this.isSetToExpire = isSetToExpire;
        }
        
        boolean isSetToExpire() {
        	return isSetToExpire;
        }
        ...
        private boolean isSetToExpire = false;
    }
    ...
    private static class ExpiryChecker implements ExpiringConcurrentCache.ExpiryChecker<URL, Record> {
        ...
        public boolean isExpired(URL url, Record record) {
            ...
            if( record.isSetToExpire() ) {
            	record.setToExpire(false);
            	return true;
            }
    ...
    private static class AlmostNeverExpired implements ExpiringConcurrentCache.ExpiryChecker<URL, Record> {
        public boolean isExpired(URL key, Record record) {
            if( record.isSetToExpire() ) {
            	record.setToExpire(false);
            	return true;
            }
